...
// Get a boot entry option information
BDS_LOAD_OPTION* GetBootEntry(LPCWSTR BootEntry, int id)
{
	...
	//Get Description
	int descSize = wstrlen((WCHAR*)DESCRIPTION_OFFSET(buffer));  //wstrlen竟然会将\0计算在内，并且其返回的char的个数，而不是wchar的个数。
	...
	if (!pBootEntry->FilePathList)
		return NULL;       //DESCRIPTION_OFFSET：计算的是description的开始位置，+ descSize得到FilePathList开始的位置。
...
//analysis if a buffer is a BDS load option
